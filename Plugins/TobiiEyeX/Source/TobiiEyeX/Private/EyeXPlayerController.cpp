...
AEyeXPlayerController::AEyeXPlayerController(const FObjectInitializer& ObjectInitializer)
{
...
AEyeXActorBase* AEyeXPlayerController::FindByLineTrace(FHitResult& OutHit, const FSceneView* const View, const FVector2D& GazePoint,
	const FCollisionObjectQueryParams& TraceParams, const FCollisionQueryParams& ObjectParams)
{
	...
	if (World->LineTraceSingleByObjectType(OutHit, Start, End, TraceParams, ObjectParams))
	{
	...
	const FCollisionObjectQueryParams& TraceParams, const FCollisionQueryParams& ObjectParams)
{
	...
	else
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (World->LineTraceSingleByObjectType(OutHit, BoxStart, BoxEnd, TraceParams, ObjectParams))
			{
	...
	const FCollisionQueryParams& TraceParams, const FCollisionObjectQueryParams& ObjectParams)
{
	...
	for (int i = 0; i < SweepIntervals; ++i)	
	{
		...
		if (World->SweepSingleByObjectType(OutHit, Start, End, FQuat::Identity, ObjectParams, Shape))
		{
			EyeXActor = Cast<AEyeXActorBase>(OutHit.GetActor());
			break;
		}
